	...
	while True:
		...
		if dg_STX == 2 and dg_ETX == 3:
			...
			if dg_ID in [73, 105]:
				...
				data['IP'][len(data['IP'])] = libs.parseEM.IP_dg(dg)
			...
			if dg_ID == 78 and not parse_params_only:
				...
				data['RRA'][len(data['RRA'])] = libs.parseEM.RRA_78_dg(dg)

			# parse POS 80 datagram
			if dg_ID == 80:
				data['POS'][len(data['POS'])] = libs.parseEM.POS_dg(dg)

			# Parse RUNTIME PARAM datagram PYTHON 3
			if dg_ID == 82:
				data['RTP'][len(data['RTP'])] = libs.parseEM.RTP_dg(dg)

			# Parse XYZ 88 datagram PYTHON 3
			if dg_ID == 88 and not skip_xyz:  # skip if not needed to store last param update

				XYZ_temp = libs.parseEM.XYZ_dg(dg, parse_outermost_only=parse_outermost_only,
